...
function withCustomGradleProperties(config) {
  return withGradleProperties(config, (config) => {
    const enableJetifier = config.modResults.find(
      (prop) => prop.key === "android.enableJetifier"
    );
    ...
    const jvmArgs = config.modResults.find(
      (prop) => prop.key === "org.gradle.jvmargs"
    );
...
module.exports = (config) =>
  withPlugins(config, [withAndroidManifestFix, withCustomGradleProperties]);
